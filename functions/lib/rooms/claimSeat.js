...
export const claimSeat = onCall({
    ...
    const { roomId, team, displayName } = req.data || {};
    if (!roomId || (team !== 'PURPLE' && team !== 'ORANGE'))
        throw new HttpsError('invalid-argument', 'INVALID_ARGUMENT');
    const name = String(displayName ?? '').trim();
    if (!name)
        throw new HttpsError('failed-precondition', 'EMPTY_DISPLAY_NAME');
    ...
    await db.runTransaction(async (tx) => {
        ...
        const other = team === 'PURPLE' ? 'ORANGE' : 'PURPLE';
        const otherUid = data?.seats?.[other]?.uid;
        if (otherUid && otherUid === uid)
            throw new HttpsError('failed-precondition', 'SAME_USER_BOTH_SEATS');
        const seat = data?.seats?.[team] ?? {};
        if (seat.uid)
            throw new HttpsError('failed-precondition', 'ALREADY_OCCUPIED');
        tx.update(ref, {
            [`seats.${team}.uid`]: uid,
