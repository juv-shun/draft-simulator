...
export const applyAction = onCall({ cors: true, region: 'asia-northeast1' }, async (req) => {
  ...
  const act = { kind: action.kind as 'ban' | 'pick', ids: action.ids as string[] } as const;
  ...
  await db.runTransaction(async (tx) => {
    ...
    const team: Team | null =
      data?.seats?.PURPLE?.uid === uid ? 'PURPLE' : data?.seats?.ORANGE?.uid === uid ? 'ORANGE' : null;
    if (!team) throw new HttpsError('permission-denied', 'NOT_SEATED');
...
function toHttpsErrorCode(msg?: string): any {
  switch (msg) {
